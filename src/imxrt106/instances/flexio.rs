...
#[allow(private_no_mangle_statics)]
#[no_mangle]
static FLEXIO3_TAKEN: AtomicBool = AtomicBool::new(false);
...
impl FLEXIO3 {
    ...
    #[inline]
    pub fn take() -> Option<Self> {
        let taken = FLEXIO3_TAKEN.swap(true, Ordering::SeqCst);
        if taken {
            None
        } else {
            Some(Self::INSTANCE)
    ...
    #[inline]
    pub fn release(_: Self) {
        let taken = FLEXIO3_TAKEN.swap(false, Ordering::SeqCst);
        assert!(taken, "Released a peripheral which was not taken");
    }
    ...
    #[inline]
    pub unsafe fn steal() -> Self {
        FLEXIO3_TAKEN.store(true, Ordering::SeqCst);
        Self::INSTANCE
    }
}

impl FLEXIO3 {
    /// The interrupts associated with FLEXIO3
    #[cfg(not(feature = "doc"))]
    pub const INTERRUPTS: [crate::Interrupt; 1] = [crate::interrupt::FLEXIO3];
