...
pub mod CR {
    ...
    pub mod Fn {
        ...
        pub mod RW {
            ...
            pub const clear: u32 = 0b000;

            /// 0b001: Sets the Fn bit in the MUA.SR register.
            pub const set: u32 = 0b001;
        }
    }
...
/// The MUB peripheral instance.
#[cfg(not(feature = "doc"))]
pub type MUB = Instance<0>;
...
#[no_mangle]
static MUB_TAKEN: AtomicBool = AtomicBool::new(false);

/// Access functions for the MUB peripheral instance
#[cfg(not(feature = "nosync"))]
impl MUB {
    const INSTANCE: Self = Self {
        addr: 0x40c4c000,
        #[cfg(not(feature = "doc"))]
        intrs: &[crate::interrupt::MUB],
    };

    /// Reset values for each field in MUB
    ...
    pub fn take() -> Option<Self> {
        let taken = MUB_TAKEN.swap(true, Ordering::SeqCst);
        if taken {
            None
        } else {
            Some(Self::INSTANCE)
    ...
    /// Release exclusive access to MUB
    ///
    ...
    pub fn release(_: Self) {
        let taken = MUB_TAKEN.swap(false, Ordering::SeqCst);
    ...
    pub unsafe fn steal() -> Self {
        MUB_TAKEN.store(true, Ordering::SeqCst);
        Self::INSTANCE
    }
}

impl MUB {
    /// The interrupts associated with MUB
    #[cfg(not(feature = "doc"))]
    pub const INTERRUPTS: [crate::Interrupt; 1] = [crate::interrupt::MUB];
...
pub const MUB: *const RegisterBlock = 0x40c4c000 as *const _;
