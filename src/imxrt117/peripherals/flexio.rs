#![allow(non_snake_case, non_upper_case_globals)]
#![allow(non_camel_case_types)]
//! FLEXIO
//!
//! Used by: imxrt1176_cm4, imxrt1176_cm7

use crate::{RORegister, RWRegister};

/// Version ID Register
pub mod VERID {

    /// Feature Specification Number
    pub mod FEATURE {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (16 bits: 0xffff << 0)
        pub const mask: u32 = 0xffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0000000000000000: Standard features implemented.
            pub const standard: u32 = 0b0000000000000000;

            /// 0b0000000000000001: Supports state, logic and parallel modes.
            pub const state_logic_parallel: u32 = 0b0000000000000001;

            /// 0b0000000000000010: Supports pin control registers.
            pub const pinctrl: u32 = 0b0000000000000010;

            /// 0b0000000000000011: Supports state, logic and parallel modes; plus pin control registers.
            pub const state_logic_parallel_pinctrl: u32 = 0b0000000000000011;
        }
    }

    /// Minor Version Number
    pub mod MINOR {
        /// Offset (16 bits)
        pub const offset: u32 = 16;
        /// Mask (8 bits: 0xff << 16)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }

    /// Major Version Number
    pub mod MAJOR {
        /// Offset (24 bits)
        pub const offset: u32 = 24;
        /// Mask (8 bits: 0xff << 24)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Parameter Register
pub mod PARAM {

    /// Shifter Number
    pub mod SHIFTER {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }

    /// Timer Number
    pub mod TIMER {
        /// Offset (8 bits)
        pub const offset: u32 = 8;
        /// Mask (8 bits: 0xff << 8)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }

    /// Pin Number
    pub mod PIN {
        /// Offset (16 bits)
        pub const offset: u32 = 16;
        /// Mask (8 bits: 0xff << 16)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }

    /// Trigger Number
    pub mod TRIGGER {
        /// Offset (24 bits)
        pub const offset: u32 = 24;
        /// Mask (8 bits: 0xff << 24)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// FlexIO Control Register
pub mod CTRL {

    /// FlexIO Enable
    pub mod FLEXEN {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (1 bit: 1 << 0)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: FlexIO module is disabled.
            pub const disable: u32 = 0b0;

            /// 0b1: FlexIO module is enabled.
            pub const enable: u32 = 0b1;
        }
    }

    /// Software Reset
    pub mod SWRST {
        /// Offset (1 bits)
        pub const offset: u32 = 1;
        /// Mask (1 bit: 1 << 1)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Software reset is disabled
            pub const disable: u32 = 0b0;

            /// 0b1: Software reset is enabled, all FlexIO registers except the Control Register are reset.
            pub const enable: u32 = 0b1;
        }
    }

    /// Fast Access
    pub mod FASTACC {
        /// Offset (2 bits)
        pub const offset: u32 = 2;
        /// Mask (1 bit: 1 << 2)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Configures for normal register accesses to FlexIO
            pub const normal: u32 = 0b0;

            /// 0b1: Configures for fast register accesses to FlexIO
            pub const fast: u32 = 0b1;
        }
    }

    /// Debug Enable
    pub mod DBGE {
        /// Offset (30 bits)
        pub const offset: u32 = 30;
        /// Mask (1 bit: 1 << 30)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: FlexIO is disabled in debug modes.
            pub const disable: u32 = 0b0;

            /// 0b1: FlexIO is enabled in debug modes
            pub const emable: u32 = 0b1;
        }
    }

    /// Doze Enable
    pub mod DOZEN {
        /// Offset (31 bits)
        pub const offset: u32 = 31;
        /// Mask (1 bit: 1 << 31)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: FlexIO enabled in Doze modes.
            pub const enable: u32 = 0b0;

            /// 0b1: FlexIO disabled in Doze modes.
            pub const disable: u32 = 0b1;
        }
    }
}

/// Pin State Register
pub mod PIN {

    /// Pin Data Input
    pub mod PDI {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Status Register
pub mod SHIFTSTAT {

    /// Shifter Status Flag
    pub mod SSF {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Error Register
pub mod SHIFTERR {

    /// Shifter Error Flags
    pub mod SEF {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Timer Status Register
pub mod TIMSTAT {

    /// Timer Status Flags
    pub mod TSF {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Status Interrupt Enable
pub mod SHIFTSIEN {

    /// Shifter Status Interrupt Enable
    pub mod SSIE {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Error Interrupt Enable
pub mod SHIFTEIEN {

    /// Shifter Error Interrupt Enable
    pub mod SEIE {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Timer Interrupt Enable Register
pub mod TIMIEN {

    /// Timer Status Interrupt Enable
    pub mod TEIE {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Status DMA Enable
pub mod SHIFTSDEN {

    /// Shifter Status DMA Enable
    pub mod SSDE {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Timer Status DMA Enable
pub mod TIMERSDEN {

    /// Timer Status DMA Enable
    pub mod TSDE {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (8 bits: 0xff << 0)
        pub const mask: u32 = 0xff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter State Register
pub mod SHIFTSTATE {

    /// Current State Pointer
    pub mod STATE {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (3 bits: 0b111 << 0)
        pub const mask: u32 = 0b111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Control N Register
pub mod SHIFTCTL0 {

    /// Shifter Mode
    pub mod SMOD {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (3 bits: 0b111 << 0)
        pub const mask: u32 = 0b111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b000: Disabled.
            pub const disable: u32 = 0b000;

            /// 0b001: Receive mode. Captures the current Shifter content into the SHIFTBUF on expiration of the Timer.
            pub const receive: u32 = 0b001;

            /// 0b010: Transmit mode. Load SHIFTBUF contents into the Shifter on expiration of the Timer.
            pub const transmit: u32 = 0b010;

            /// 0b100: Match Store mode. Shifter data is compared to SHIFTBUF content on expiration of the Timer.
            pub const matchstore: u32 = 0b100;

            /// 0b101: Match Continuous mode. Shifter data is continuously compared to SHIFTBUF contents.
            pub const matchcont: u32 = 0b101;

            /// 0b110: State mode. SHIFTBUF contents are used for storing programmable state attributes.
            pub const state: u32 = 0b110;

            /// 0b111: Logic mode. SHIFTBUF contents are used for implementing programmable logic look up table.
            pub const logic: u32 = 0b111;
        }
    }

    /// Shifter Pin Polarity
    pub mod PINPOL {
        /// Offset (7 bits)
        pub const offset: u32 = 7;
        /// Mask (1 bit: 1 << 7)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Pin is active high
            pub const active_high: u32 = 0b0;

            /// 0b1: Pin is active low
            pub const active_low: u32 = 0b1;
        }
    }

    /// Shifter Pin Select
    pub mod PINSEL {
        /// Offset (8 bits)
        pub const offset: u32 = 8;
        /// Mask (5 bits: 0b11111 << 8)
        pub const mask: u32 = 0b11111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }

    /// Shifter Pin Configuration
    pub mod PINCFG {
        /// Offset (16 bits)
        pub const offset: u32 = 16;
        /// Mask (2 bits: 0b11 << 16)
        pub const mask: u32 = 0b11 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b00: Shifter pin output disabled
            pub const disable: u32 = 0b00;

            /// 0b01: Shifter pin open drain or bidirectional output enable
            pub const opend_bidirouten: u32 = 0b01;

            /// 0b10: Shifter pin bidirectional output data
            pub const bidir_outdata: u32 = 0b10;

            /// 0b11: Shifter pin output
            pub const output: u32 = 0b11;
        }
    }

    /// Timer Polarity
    pub mod TIMPOL {
        /// Offset (23 bits)
        pub const offset: u32 = 23;
        /// Mask (1 bit: 1 << 23)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Shift on posedge of Shift clock
            pub const posedge: u32 = 0b0;

            /// 0b1: Shift on negedge of Shift clock
            pub const negedge: u32 = 0b1;
        }
    }

    /// Timer Select
    pub mod TIMSEL {
        /// Offset (24 bits)
        pub const offset: u32 = 24;
        /// Mask (3 bits: 0b111 << 24)
        pub const mask: u32 = 0b111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Control N Register
pub mod SHIFTCTL1 {
    pub use super::SHIFTCTL0::PINCFG;
    pub use super::SHIFTCTL0::PINPOL;
    pub use super::SHIFTCTL0::PINSEL;
    pub use super::SHIFTCTL0::SMOD;
    pub use super::SHIFTCTL0::TIMPOL;
    pub use super::SHIFTCTL0::TIMSEL;
}

/// Shifter Control N Register
pub mod SHIFTCTL2 {
    pub use super::SHIFTCTL0::PINCFG;
    pub use super::SHIFTCTL0::PINPOL;
    pub use super::SHIFTCTL0::PINSEL;
    pub use super::SHIFTCTL0::SMOD;
    pub use super::SHIFTCTL0::TIMPOL;
    pub use super::SHIFTCTL0::TIMSEL;
}

/// Shifter Control N Register
pub mod SHIFTCTL3 {
    pub use super::SHIFTCTL0::PINCFG;
    pub use super::SHIFTCTL0::PINPOL;
    pub use super::SHIFTCTL0::PINSEL;
    pub use super::SHIFTCTL0::SMOD;
    pub use super::SHIFTCTL0::TIMPOL;
    pub use super::SHIFTCTL0::TIMSEL;
}

/// Shifter Control N Register
pub mod SHIFTCTL4 {
    pub use super::SHIFTCTL0::PINCFG;
    pub use super::SHIFTCTL0::PINPOL;
    pub use super::SHIFTCTL0::PINSEL;
    pub use super::SHIFTCTL0::SMOD;
    pub use super::SHIFTCTL0::TIMPOL;
    pub use super::SHIFTCTL0::TIMSEL;
}

/// Shifter Control N Register
pub mod SHIFTCTL5 {
    pub use super::SHIFTCTL0::PINCFG;
    pub use super::SHIFTCTL0::PINPOL;
    pub use super::SHIFTCTL0::PINSEL;
    pub use super::SHIFTCTL0::SMOD;
    pub use super::SHIFTCTL0::TIMPOL;
    pub use super::SHIFTCTL0::TIMSEL;
}

/// Shifter Control N Register
pub mod SHIFTCTL6 {
    pub use super::SHIFTCTL0::PINCFG;
    pub use super::SHIFTCTL0::PINPOL;
    pub use super::SHIFTCTL0::PINSEL;
    pub use super::SHIFTCTL0::SMOD;
    pub use super::SHIFTCTL0::TIMPOL;
    pub use super::SHIFTCTL0::TIMSEL;
}

/// Shifter Control N Register
pub mod SHIFTCTL7 {
    pub use super::SHIFTCTL0::PINCFG;
    pub use super::SHIFTCTL0::PINPOL;
    pub use super::SHIFTCTL0::PINSEL;
    pub use super::SHIFTCTL0::SMOD;
    pub use super::SHIFTCTL0::TIMPOL;
    pub use super::SHIFTCTL0::TIMSEL;
}

/// Shifter Configuration N Register
pub mod SHIFTCFG0 {

    /// Shifter Start bit
    pub mod SSTART {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (2 bits: 0b11 << 0)
        pub const mask: u32 = 0b11 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b00: Start bit disabled for transmitter/receiver/match store, transmitter loads data on enable
            pub const value00: u32 = 0b00;

            /// 0b01: Start bit disabled for transmitter/receiver/match store, transmitter loads data on first shift
            pub const value01: u32 = 0b01;

            /// 0b10: Transmitter outputs start bit value 0 before loading data on first shift, receiver/match store sets error flag if start bit is not 0
            pub const value10: u32 = 0b10;

            /// 0b11: Transmitter outputs start bit value 1 before loading data on first shift, receiver/match store sets error flag if start bit is not 1
            pub const value11: u32 = 0b11;
        }
    }

    /// Shifter Stop bit
    pub mod SSTOP {
        /// Offset (4 bits)
        pub const offset: u32 = 4;
        /// Mask (2 bits: 0b11 << 4)
        pub const mask: u32 = 0b11 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b00: Stop bit disabled for transmitter/receiver/match store
            pub const value00: u32 = 0b00;

            /// 0b10: Transmitter outputs stop bit value 0 on store, receiver/match store sets error flag if stop bit is not 0
            pub const value10: u32 = 0b10;

            /// 0b11: Transmitter outputs stop bit value 1 on store, receiver/match store sets error flag if stop bit is not 1
            pub const value11: u32 = 0b11;
        }
    }

    /// Input Source
    pub mod INSRC {
        /// Offset (8 bits)
        pub const offset: u32 = 8;
        /// Mask (1 bit: 1 << 8)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Pin
            pub const pin: u32 = 0b0;

            /// 0b1: Shifter N+1 Output
            pub const shifter_nplus1: u32 = 0b1;
        }
    }

    /// Late Store
    pub mod LATST {
        /// Offset (9 bits)
        pub const offset: u32 = 9;
        /// Mask (1 bit: 1 << 9)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Shift register stores the pre-shift register state.
            pub const preshift: u32 = 0b0;

            /// 0b1: Shift register stores the post-shift register state.
            pub const postshift: u32 = 0b1;
        }
    }

    /// Parallel Width
    pub mod PWIDTH {
        /// Offset (16 bits)
        pub const offset: u32 = 16;
        /// Mask (5 bits: 0b11111 << 16)
        pub const mask: u32 = 0b11111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Configuration N Register
pub mod SHIFTCFG1 {
    pub use super::SHIFTCFG0::INSRC;
    pub use super::SHIFTCFG0::LATST;
    pub use super::SHIFTCFG0::PWIDTH;
    pub use super::SHIFTCFG0::SSTART;
    pub use super::SHIFTCFG0::SSTOP;
}

/// Shifter Configuration N Register
pub mod SHIFTCFG2 {
    pub use super::SHIFTCFG0::INSRC;
    pub use super::SHIFTCFG0::LATST;
    pub use super::SHIFTCFG0::PWIDTH;
    pub use super::SHIFTCFG0::SSTART;
    pub use super::SHIFTCFG0::SSTOP;
}

/// Shifter Configuration N Register
pub mod SHIFTCFG3 {
    pub use super::SHIFTCFG0::INSRC;
    pub use super::SHIFTCFG0::LATST;
    pub use super::SHIFTCFG0::PWIDTH;
    pub use super::SHIFTCFG0::SSTART;
    pub use super::SHIFTCFG0::SSTOP;
}

/// Shifter Configuration N Register
pub mod SHIFTCFG4 {
    pub use super::SHIFTCFG0::INSRC;
    pub use super::SHIFTCFG0::LATST;
    pub use super::SHIFTCFG0::PWIDTH;
    pub use super::SHIFTCFG0::SSTART;
    pub use super::SHIFTCFG0::SSTOP;
}

/// Shifter Configuration N Register
pub mod SHIFTCFG5 {
    pub use super::SHIFTCFG0::INSRC;
    pub use super::SHIFTCFG0::LATST;
    pub use super::SHIFTCFG0::PWIDTH;
    pub use super::SHIFTCFG0::SSTART;
    pub use super::SHIFTCFG0::SSTOP;
}

/// Shifter Configuration N Register
pub mod SHIFTCFG6 {
    pub use super::SHIFTCFG0::INSRC;
    pub use super::SHIFTCFG0::LATST;
    pub use super::SHIFTCFG0::PWIDTH;
    pub use super::SHIFTCFG0::SSTART;
    pub use super::SHIFTCFG0::SSTOP;
}

/// Shifter Configuration N Register
pub mod SHIFTCFG7 {
    pub use super::SHIFTCFG0::INSRC;
    pub use super::SHIFTCFG0::LATST;
    pub use super::SHIFTCFG0::PWIDTH;
    pub use super::SHIFTCFG0::SSTART;
    pub use super::SHIFTCFG0::SSTOP;
}

/// Shifter Buffer N Register
pub mod SHIFTBUF0 {

    /// Shift Buffer
    pub mod SHIFTBUF {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Register
pub mod SHIFTBUF1 {
    pub use super::SHIFTBUF0::SHIFTBUF;
}

/// Shifter Buffer N Register
pub mod SHIFTBUF2 {
    pub use super::SHIFTBUF0::SHIFTBUF;
}

/// Shifter Buffer N Register
pub mod SHIFTBUF3 {
    pub use super::SHIFTBUF0::SHIFTBUF;
}

/// Shifter Buffer N Register
pub mod SHIFTBUF4 {
    pub use super::SHIFTBUF0::SHIFTBUF;
}

/// Shifter Buffer N Register
pub mod SHIFTBUF5 {
    pub use super::SHIFTBUF0::SHIFTBUF;
}

/// Shifter Buffer N Register
pub mod SHIFTBUF6 {
    pub use super::SHIFTBUF0::SHIFTBUF;
}

/// Shifter Buffer N Register
pub mod SHIFTBUF7 {
    pub use super::SHIFTBUF0::SHIFTBUF;
}

/// Shifter Buffer N Bit Swapped Register
pub mod SHIFTBUFBIS0 {

    /// Shift Buffer
    pub mod SHIFTBUFBIS {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Bit Swapped Register
pub mod SHIFTBUFBIS1 {
    pub use super::SHIFTBUFBIS0::SHIFTBUFBIS;
}

/// Shifter Buffer N Bit Swapped Register
pub mod SHIFTBUFBIS2 {
    pub use super::SHIFTBUFBIS0::SHIFTBUFBIS;
}

/// Shifter Buffer N Bit Swapped Register
pub mod SHIFTBUFBIS3 {
    pub use super::SHIFTBUFBIS0::SHIFTBUFBIS;
}

/// Shifter Buffer N Bit Swapped Register
pub mod SHIFTBUFBIS4 {
    pub use super::SHIFTBUFBIS0::SHIFTBUFBIS;
}

/// Shifter Buffer N Bit Swapped Register
pub mod SHIFTBUFBIS5 {
    pub use super::SHIFTBUFBIS0::SHIFTBUFBIS;
}

/// Shifter Buffer N Bit Swapped Register
pub mod SHIFTBUFBIS6 {
    pub use super::SHIFTBUFBIS0::SHIFTBUFBIS;
}

/// Shifter Buffer N Bit Swapped Register
pub mod SHIFTBUFBIS7 {
    pub use super::SHIFTBUFBIS0::SHIFTBUFBIS;
}

/// Shifter Buffer N Byte Swapped Register
pub mod SHIFTBUFBYS0 {

    /// Shift Buffer
    pub mod SHIFTBUFBYS {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Byte Swapped Register
pub mod SHIFTBUFBYS1 {
    pub use super::SHIFTBUFBYS0::SHIFTBUFBYS;
}

/// Shifter Buffer N Byte Swapped Register
pub mod SHIFTBUFBYS2 {
    pub use super::SHIFTBUFBYS0::SHIFTBUFBYS;
}

/// Shifter Buffer N Byte Swapped Register
pub mod SHIFTBUFBYS3 {
    pub use super::SHIFTBUFBYS0::SHIFTBUFBYS;
}

/// Shifter Buffer N Byte Swapped Register
pub mod SHIFTBUFBYS4 {
    pub use super::SHIFTBUFBYS0::SHIFTBUFBYS;
}

/// Shifter Buffer N Byte Swapped Register
pub mod SHIFTBUFBYS5 {
    pub use super::SHIFTBUFBYS0::SHIFTBUFBYS;
}

/// Shifter Buffer N Byte Swapped Register
pub mod SHIFTBUFBYS6 {
    pub use super::SHIFTBUFBYS0::SHIFTBUFBYS;
}

/// Shifter Buffer N Byte Swapped Register
pub mod SHIFTBUFBYS7 {
    pub use super::SHIFTBUFBYS0::SHIFTBUFBYS;
}

/// Shifter Buffer N Bit Byte Swapped Register
pub mod SHIFTBUFBBS0 {

    /// Shift Buffer
    pub mod SHIFTBUFBBS {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Bit Byte Swapped Register
pub mod SHIFTBUFBBS1 {
    pub use super::SHIFTBUFBBS0::SHIFTBUFBBS;
}

/// Shifter Buffer N Bit Byte Swapped Register
pub mod SHIFTBUFBBS2 {
    pub use super::SHIFTBUFBBS0::SHIFTBUFBBS;
}

/// Shifter Buffer N Bit Byte Swapped Register
pub mod SHIFTBUFBBS3 {
    pub use super::SHIFTBUFBBS0::SHIFTBUFBBS;
}

/// Shifter Buffer N Bit Byte Swapped Register
pub mod SHIFTBUFBBS4 {
    pub use super::SHIFTBUFBBS0::SHIFTBUFBBS;
}

/// Shifter Buffer N Bit Byte Swapped Register
pub mod SHIFTBUFBBS5 {
    pub use super::SHIFTBUFBBS0::SHIFTBUFBBS;
}

/// Shifter Buffer N Bit Byte Swapped Register
pub mod SHIFTBUFBBS6 {
    pub use super::SHIFTBUFBBS0::SHIFTBUFBBS;
}

/// Shifter Buffer N Bit Byte Swapped Register
pub mod SHIFTBUFBBS7 {
    pub use super::SHIFTBUFBBS0::SHIFTBUFBBS;
}

/// Timer Control N Register
pub mod TIMCTL0 {

    /// Timer Mode
    pub mod TIMOD {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (3 bits: 0b111 << 0)
        pub const mask: u32 = 0b111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b000: Timer Disabled.
            pub const disable: u32 = 0b000;

            /// 0b001: Dual 8-bit counters baud mode.
            pub const dual8bit_baud: u32 = 0b001;

            /// 0b010: Dual 8-bit counters PWM high mode.
            pub const dual8bit_pwm_h: u32 = 0b010;

            /// 0b011: Single 16-bit counter mode.
            pub const single16bit: u32 = 0b011;

            /// 0b100: Single 16-bit counter disable mode.
            pub const single16bit_disable: u32 = 0b100;

            /// 0b101: Dual 8-bit counters word mode.
            pub const dual8bit_word: u32 = 0b101;

            /// 0b110: Dual 8-bit counters PWM low mode.
            pub const dual8bit_pwm_l: u32 = 0b110;

            /// 0b111: Single 16-bit input capture mode.
            pub const single16bit_in_capture: u32 = 0b111;
        }
    }

    /// Timer One Time Operation
    pub mod ONETIM {
        /// Offset (5 bits)
        pub const offset: u32 = 5;
        /// Mask (1 bit: 1 << 5)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: The timer enable event is generated as normal.
            pub const not_blocked: u32 = 0b0;

            /// 0b1: The timer enable event is blocked unless timer status flag is clear.
            pub const blocked: u32 = 0b1;
        }
    }

    /// Timer Pin Input Select
    pub mod PININS {
        /// Offset (6 bits)
        pub const offset: u32 = 6;
        /// Mask (1 bit: 1 << 6)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Timer pin input and output are selected by PINSEL.
            pub const pinsel: u32 = 0b0;

            /// 0b1: Timer pin input is selected by PINSEL+1, timer pin output remains selected by PINSEL.
            pub const pinselplus1: u32 = 0b1;
        }
    }

    /// Timer Pin Polarity
    pub mod PINPOL {
        /// Offset (7 bits)
        pub const offset: u32 = 7;
        /// Mask (1 bit: 1 << 7)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Pin is active high
            pub const active_high: u32 = 0b0;

            /// 0b1: Pin is active low
            pub const active_low: u32 = 0b1;
        }
    }

    /// Timer Pin Select
    pub mod PINSEL {
        /// Offset (8 bits)
        pub const offset: u32 = 8;
        /// Mask (5 bits: 0b11111 << 8)
        pub const mask: u32 = 0b11111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }

    /// Timer Pin Configuration
    pub mod PINCFG {
        /// Offset (16 bits)
        pub const offset: u32 = 16;
        /// Mask (2 bits: 0b11 << 16)
        pub const mask: u32 = 0b11 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b00: Timer pin output disabled
            pub const outdisable: u32 = 0b00;

            /// 0b01: Timer pin open drain or bidirectional output enable
            pub const opend_bidirouten: u32 = 0b01;

            /// 0b10: Timer pin bidirectional output data
            pub const bidir_outdata: u32 = 0b10;

            /// 0b11: Timer pin output
            pub const output: u32 = 0b11;
        }
    }

    /// Trigger Source
    pub mod TRGSRC {
        /// Offset (22 bits)
        pub const offset: u32 = 22;
        /// Mask (1 bit: 1 << 22)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: External trigger selected
            pub const ext_trig: u32 = 0b0;

            /// 0b1: Internal trigger selected
            pub const internal_trig: u32 = 0b1;
        }
    }

    /// Trigger Polarity
    pub mod TRGPOL {
        /// Offset (23 bits)
        pub const offset: u32 = 23;
        /// Mask (1 bit: 1 << 23)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Trigger active high
            pub const active_high: u32 = 0b0;

            /// 0b1: Trigger active low
            pub const active_low: u32 = 0b1;
        }
    }

    /// Trigger Select
    pub mod TRGSEL {
        /// Offset (24 bits)
        pub const offset: u32 = 24;
        /// Mask (6 bits: 0x3f << 24)
        pub const mask: u32 = 0x3f << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Timer Control N Register
pub mod TIMCTL1 {
    pub use super::TIMCTL0::ONETIM;
    pub use super::TIMCTL0::PINCFG;
    pub use super::TIMCTL0::PININS;
    pub use super::TIMCTL0::PINPOL;
    pub use super::TIMCTL0::PINSEL;
    pub use super::TIMCTL0::TIMOD;
    pub use super::TIMCTL0::TRGPOL;
    pub use super::TIMCTL0::TRGSEL;
    pub use super::TIMCTL0::TRGSRC;
}

/// Timer Control N Register
pub mod TIMCTL2 {
    pub use super::TIMCTL0::ONETIM;
    pub use super::TIMCTL0::PINCFG;
    pub use super::TIMCTL0::PININS;
    pub use super::TIMCTL0::PINPOL;
    pub use super::TIMCTL0::PINSEL;
    pub use super::TIMCTL0::TIMOD;
    pub use super::TIMCTL0::TRGPOL;
    pub use super::TIMCTL0::TRGSEL;
    pub use super::TIMCTL0::TRGSRC;
}

/// Timer Control N Register
pub mod TIMCTL3 {
    pub use super::TIMCTL0::ONETIM;
    pub use super::TIMCTL0::PINCFG;
    pub use super::TIMCTL0::PININS;
    pub use super::TIMCTL0::PINPOL;
    pub use super::TIMCTL0::PINSEL;
    pub use super::TIMCTL0::TIMOD;
    pub use super::TIMCTL0::TRGPOL;
    pub use super::TIMCTL0::TRGSEL;
    pub use super::TIMCTL0::TRGSRC;
}

/// Timer Control N Register
pub mod TIMCTL4 {
    pub use super::TIMCTL0::ONETIM;
    pub use super::TIMCTL0::PINCFG;
    pub use super::TIMCTL0::PININS;
    pub use super::TIMCTL0::PINPOL;
    pub use super::TIMCTL0::PINSEL;
    pub use super::TIMCTL0::TIMOD;
    pub use super::TIMCTL0::TRGPOL;
    pub use super::TIMCTL0::TRGSEL;
    pub use super::TIMCTL0::TRGSRC;
}

/// Timer Control N Register
pub mod TIMCTL5 {
    pub use super::TIMCTL0::ONETIM;
    pub use super::TIMCTL0::PINCFG;
    pub use super::TIMCTL0::PININS;
    pub use super::TIMCTL0::PINPOL;
    pub use super::TIMCTL0::PINSEL;
    pub use super::TIMCTL0::TIMOD;
    pub use super::TIMCTL0::TRGPOL;
    pub use super::TIMCTL0::TRGSEL;
    pub use super::TIMCTL0::TRGSRC;
}

/// Timer Control N Register
pub mod TIMCTL6 {
    pub use super::TIMCTL0::ONETIM;
    pub use super::TIMCTL0::PINCFG;
    pub use super::TIMCTL0::PININS;
    pub use super::TIMCTL0::PINPOL;
    pub use super::TIMCTL0::PINSEL;
    pub use super::TIMCTL0::TIMOD;
    pub use super::TIMCTL0::TRGPOL;
    pub use super::TIMCTL0::TRGSEL;
    pub use super::TIMCTL0::TRGSRC;
}

/// Timer Control N Register
pub mod TIMCTL7 {
    pub use super::TIMCTL0::ONETIM;
    pub use super::TIMCTL0::PINCFG;
    pub use super::TIMCTL0::PININS;
    pub use super::TIMCTL0::PINPOL;
    pub use super::TIMCTL0::PINSEL;
    pub use super::TIMCTL0::TIMOD;
    pub use super::TIMCTL0::TRGPOL;
    pub use super::TIMCTL0::TRGSEL;
    pub use super::TIMCTL0::TRGSRC;
}

/// Timer Configuration N Register
pub mod TIMCFG0 {

    /// Timer Start Bit
    pub mod TSTART {
        /// Offset (1 bits)
        pub const offset: u32 = 1;
        /// Mask (1 bit: 1 << 1)
        pub const mask: u32 = 1 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b0: Start bit disabled
            pub const disable: u32 = 0b0;

            /// 0b1: Start bit enabled
            pub const enable: u32 = 0b1;
        }
    }

    /// Timer Stop Bit
    pub mod TSTOP {
        /// Offset (4 bits)
        pub const offset: u32 = 4;
        /// Mask (2 bits: 0b11 << 4)
        pub const mask: u32 = 0b11 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b00: Stop bit disabled
            pub const stop_disable: u32 = 0b00;

            /// 0b01: Stop bit is enabled on timer compare
            pub const enable_tmrcmp: u32 = 0b01;

            /// 0b10: Stop bit is enabled on timer disable
            pub const enable_tmrdisable: u32 = 0b10;

            /// 0b11: Stop bit is enabled on timer compare and timer disable
            pub const enable_tmr_cmp_dis: u32 = 0b11;
        }
    }

    /// Timer Enable
    pub mod TIMENA {
        /// Offset (8 bits)
        pub const offset: u32 = 8;
        /// Mask (3 bits: 0b111 << 8)
        pub const mask: u32 = 0b111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b000: Timer always enabled
            pub const enable: u32 = 0b000;

            /// 0b001: Timer enabled on Timer N-1 enable
            pub const tmr_nminus1_en: u32 = 0b001;

            /// 0b010: Timer enabled on Trigger high
            pub const tmr_trighi_en: u32 = 0b010;

            /// 0b011: Timer enabled on Trigger high and Pin high
            pub const tmr_trig_pin_hi_en: u32 = 0b011;

            /// 0b100: Timer enabled on Pin rising edge
            pub const tmr_pinrise_en: u32 = 0b100;

            /// 0b101: Timer enabled on Pin rising edge and Trigger high
            pub const tmr_pinrise_trighi_en: u32 = 0b101;

            /// 0b110: Timer enabled on Trigger rising edge
            pub const tmr_trigrise_en: u32 = 0b110;

            /// 0b111: Timer enabled on Trigger rising or falling edge
            pub const tmr_trigedge_en: u32 = 0b111;
        }
    }

    /// Timer Disable
    pub mod TIMDIS {
        /// Offset (12 bits)
        pub const offset: u32 = 12;
        /// Mask (3 bits: 0b111 << 12)
        pub const mask: u32 = 0b111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b000: Timer never disabled
            pub const never: u32 = 0b000;

            /// 0b001: Timer disabled on Timer N-1 disable
            pub const tmr_nminus1: u32 = 0b001;

            /// 0b010: Timer disabled on Timer compare (upper 8-bits match and decrement)
            pub const tmr_cmp: u32 = 0b010;

            /// 0b011: Timer disabled on Timer compare (upper 8-bits match and decrement) and Trigger Low
            pub const tmr_cmp_triglow: u32 = 0b011;

            /// 0b100: Timer disabled on Pin rising or falling edge
            pub const pin_edge: u32 = 0b100;

            /// 0b101: Timer disabled on Pin rising or falling edge provided Trigger is high
            pub const pin_edge_trighi: u32 = 0b101;

            /// 0b110: Timer disabled on Trigger falling edge
            pub const trig_falledge: u32 = 0b110;
        }
    }

    /// Timer Reset
    pub mod TIMRST {
        /// Offset (16 bits)
        pub const offset: u32 = 16;
        /// Mask (3 bits: 0b111 << 16)
        pub const mask: u32 = 0b111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b000: Timer never reset
            pub const never: u32 = 0b000;

            /// 0b001: Timer reset on Timer Output high.
            pub const tmr_out_hi: u32 = 0b001;

            /// 0b010: Timer reset on Timer Pin equal to Timer Output
            pub const pin_eq_tmr_out: u32 = 0b010;

            /// 0b011: Timer reset on Timer Trigger equal to Timer Output
            pub const trig_eq_tmr_out: u32 = 0b011;

            /// 0b100: Timer reset on Timer Pin rising edge
            pub const pin_rise_edge: u32 = 0b100;

            /// 0b110: Timer reset on Trigger rising edge
            pub const trig_rise_edge: u32 = 0b110;

            /// 0b111: Timer reset on Trigger rising or falling edge
            pub const trig_edge: u32 = 0b111;
        }
    }

    /// Timer Decrement
    pub mod TIMDEC {
        /// Offset (20 bits)
        pub const offset: u32 = 20;
        /// Mask (3 bits: 0b111 << 20)
        pub const mask: u32 = 0b111 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b000: Decrement counter on FlexIO clock, Shift clock equals Timer output.
            pub const flexio_clk_shiftclk_tmr_out: u32 = 0b000;

            /// 0b001: Decrement counter on Trigger input (both edges), Shift clock equals Timer output.
            pub const trig_edge_shiftclk_tmr_out: u32 = 0b001;

            /// 0b010: Decrement counter on Pin input (both edges), Shift clock equals Pin input.
            pub const pin_edge_shiftclk_tmr_out: u32 = 0b010;

            /// 0b011: Decrement counter on Trigger input (both edges), Shift clock equals Trigger input.
            pub const trig_edge_shiftclk_trig_in: u32 = 0b011;

            /// 0b100: Decrement counter on FlexIO clock divided by 16, Shift clock equals Timer output.
            pub const flexio_clk_div16_shiftclk_tmr_out: u32 = 0b100;

            /// 0b101: Decrement counter on FlexIO clock divided by 256, Shift clock equals Timer output.
            pub const flexio_clk_div256_shiftclk_tmr_out: u32 = 0b101;

            /// 0b110: Decrement counter on Pin input (rising edge), Shift clock equals Pin input.
            pub const pin_rise_shiftclk_pin_in: u32 = 0b110;

            /// 0b111: Decrement counter on Trigger input (rising edge), Shift clock equals Trigger input.
            pub const trig_rise_shiftclk_trig_in: u32 = 0b111;
        }
    }

    /// Timer Output
    pub mod TIMOUT {
        /// Offset (24 bits)
        pub const offset: u32 = 24;
        /// Mask (2 bits: 0b11 << 24)
        pub const mask: u32 = 0b11 << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values
        pub mod RW {

            /// 0b00: Timer output is logic one when enabled and is not affected by timer reset
            pub const one: u32 = 0b00;

            /// 0b01: Timer output is logic zero when enabled and is not affected by timer reset
            pub const zero: u32 = 0b01;

            /// 0b10: Timer output is logic one when enabled and on timer reset
            pub const one_tmrreset: u32 = 0b10;

            /// 0b11: Timer output is logic zero when enabled and on timer reset
            pub const zero_tmrreset: u32 = 0b11;
        }
    }
}

/// Timer Configuration N Register
pub mod TIMCFG1 {
    pub use super::TIMCFG0::TIMDEC;
    pub use super::TIMCFG0::TIMDIS;
    pub use super::TIMCFG0::TIMENA;
    pub use super::TIMCFG0::TIMOUT;
    pub use super::TIMCFG0::TIMRST;
    pub use super::TIMCFG0::TSTART;
    pub use super::TIMCFG0::TSTOP;
}

/// Timer Configuration N Register
pub mod TIMCFG2 {
    pub use super::TIMCFG0::TIMDEC;
    pub use super::TIMCFG0::TIMDIS;
    pub use super::TIMCFG0::TIMENA;
    pub use super::TIMCFG0::TIMOUT;
    pub use super::TIMCFG0::TIMRST;
    pub use super::TIMCFG0::TSTART;
    pub use super::TIMCFG0::TSTOP;
}

/// Timer Configuration N Register
pub mod TIMCFG3 {
    pub use super::TIMCFG0::TIMDEC;
    pub use super::TIMCFG0::TIMDIS;
    pub use super::TIMCFG0::TIMENA;
    pub use super::TIMCFG0::TIMOUT;
    pub use super::TIMCFG0::TIMRST;
    pub use super::TIMCFG0::TSTART;
    pub use super::TIMCFG0::TSTOP;
}

/// Timer Configuration N Register
pub mod TIMCFG4 {
    pub use super::TIMCFG0::TIMDEC;
    pub use super::TIMCFG0::TIMDIS;
    pub use super::TIMCFG0::TIMENA;
    pub use super::TIMCFG0::TIMOUT;
    pub use super::TIMCFG0::TIMRST;
    pub use super::TIMCFG0::TSTART;
    pub use super::TIMCFG0::TSTOP;
}

/// Timer Configuration N Register
pub mod TIMCFG5 {
    pub use super::TIMCFG0::TIMDEC;
    pub use super::TIMCFG0::TIMDIS;
    pub use super::TIMCFG0::TIMENA;
    pub use super::TIMCFG0::TIMOUT;
    pub use super::TIMCFG0::TIMRST;
    pub use super::TIMCFG0::TSTART;
    pub use super::TIMCFG0::TSTOP;
}

/// Timer Configuration N Register
pub mod TIMCFG6 {
    pub use super::TIMCFG0::TIMDEC;
    pub use super::TIMCFG0::TIMDIS;
    pub use super::TIMCFG0::TIMENA;
    pub use super::TIMCFG0::TIMOUT;
    pub use super::TIMCFG0::TIMRST;
    pub use super::TIMCFG0::TSTART;
    pub use super::TIMCFG0::TSTOP;
}

/// Timer Configuration N Register
pub mod TIMCFG7 {
    pub use super::TIMCFG0::TIMDEC;
    pub use super::TIMCFG0::TIMDIS;
    pub use super::TIMCFG0::TIMENA;
    pub use super::TIMCFG0::TIMOUT;
    pub use super::TIMCFG0::TIMRST;
    pub use super::TIMCFG0::TSTART;
    pub use super::TIMCFG0::TSTOP;
}

/// Timer Compare N Register
pub mod TIMCMP0 {

    /// Timer Compare Value
    pub mod CMP {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (16 bits: 0xffff << 0)
        pub const mask: u32 = 0xffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Timer Compare N Register
pub mod TIMCMP1 {
    pub use super::TIMCMP0::CMP;
}

/// Timer Compare N Register
pub mod TIMCMP2 {
    pub use super::TIMCMP0::CMP;
}

/// Timer Compare N Register
pub mod TIMCMP3 {
    pub use super::TIMCMP0::CMP;
}

/// Timer Compare N Register
pub mod TIMCMP4 {
    pub use super::TIMCMP0::CMP;
}

/// Timer Compare N Register
pub mod TIMCMP5 {
    pub use super::TIMCMP0::CMP;
}

/// Timer Compare N Register
pub mod TIMCMP6 {
    pub use super::TIMCMP0::CMP;
}

/// Timer Compare N Register
pub mod TIMCMP7 {
    pub use super::TIMCMP0::CMP;
}

/// Shifter Buffer N Nibble Byte Swapped Register
pub mod SHIFTBUFNBS0 {

    /// Shift Buffer
    pub mod SHIFTBUFNBS {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Nibble Byte Swapped Register
pub mod SHIFTBUFNBS1 {
    pub use super::SHIFTBUFNBS0::SHIFTBUFNBS;
}

/// Shifter Buffer N Nibble Byte Swapped Register
pub mod SHIFTBUFNBS2 {
    pub use super::SHIFTBUFNBS0::SHIFTBUFNBS;
}

/// Shifter Buffer N Nibble Byte Swapped Register
pub mod SHIFTBUFNBS3 {
    pub use super::SHIFTBUFNBS0::SHIFTBUFNBS;
}

/// Shifter Buffer N Nibble Byte Swapped Register
pub mod SHIFTBUFNBS4 {
    pub use super::SHIFTBUFNBS0::SHIFTBUFNBS;
}

/// Shifter Buffer N Nibble Byte Swapped Register
pub mod SHIFTBUFNBS5 {
    pub use super::SHIFTBUFNBS0::SHIFTBUFNBS;
}

/// Shifter Buffer N Nibble Byte Swapped Register
pub mod SHIFTBUFNBS6 {
    pub use super::SHIFTBUFNBS0::SHIFTBUFNBS;
}

/// Shifter Buffer N Nibble Byte Swapped Register
pub mod SHIFTBUFNBS7 {
    pub use super::SHIFTBUFNBS0::SHIFTBUFNBS;
}

/// Shifter Buffer N Half Word Swapped Register
pub mod SHIFTBUFHWS0 {

    /// Shift Buffer
    pub mod SHIFTBUFHWS {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Half Word Swapped Register
pub mod SHIFTBUFHWS1 {
    pub use super::SHIFTBUFHWS0::SHIFTBUFHWS;
}

/// Shifter Buffer N Half Word Swapped Register
pub mod SHIFTBUFHWS2 {
    pub use super::SHIFTBUFHWS0::SHIFTBUFHWS;
}

/// Shifter Buffer N Half Word Swapped Register
pub mod SHIFTBUFHWS3 {
    pub use super::SHIFTBUFHWS0::SHIFTBUFHWS;
}

/// Shifter Buffer N Half Word Swapped Register
pub mod SHIFTBUFHWS4 {
    pub use super::SHIFTBUFHWS0::SHIFTBUFHWS;
}

/// Shifter Buffer N Half Word Swapped Register
pub mod SHIFTBUFHWS5 {
    pub use super::SHIFTBUFHWS0::SHIFTBUFHWS;
}

/// Shifter Buffer N Half Word Swapped Register
pub mod SHIFTBUFHWS6 {
    pub use super::SHIFTBUFHWS0::SHIFTBUFHWS;
}

/// Shifter Buffer N Half Word Swapped Register
pub mod SHIFTBUFHWS7 {
    pub use super::SHIFTBUFHWS0::SHIFTBUFHWS;
}

/// Shifter Buffer N Nibble Swapped Register
pub mod SHIFTBUFNIS0 {

    /// Shift Buffer
    pub mod SHIFTBUFNIS {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Nibble Swapped Register
pub mod SHIFTBUFNIS1 {
    pub use super::SHIFTBUFNIS0::SHIFTBUFNIS;
}

/// Shifter Buffer N Nibble Swapped Register
pub mod SHIFTBUFNIS2 {
    pub use super::SHIFTBUFNIS0::SHIFTBUFNIS;
}

/// Shifter Buffer N Nibble Swapped Register
pub mod SHIFTBUFNIS3 {
    pub use super::SHIFTBUFNIS0::SHIFTBUFNIS;
}

/// Shifter Buffer N Nibble Swapped Register
pub mod SHIFTBUFNIS4 {
    pub use super::SHIFTBUFNIS0::SHIFTBUFNIS;
}

/// Shifter Buffer N Nibble Swapped Register
pub mod SHIFTBUFNIS5 {
    pub use super::SHIFTBUFNIS0::SHIFTBUFNIS;
}

/// Shifter Buffer N Nibble Swapped Register
pub mod SHIFTBUFNIS6 {
    pub use super::SHIFTBUFNIS0::SHIFTBUFNIS;
}

/// Shifter Buffer N Nibble Swapped Register
pub mod SHIFTBUFNIS7 {
    pub use super::SHIFTBUFNIS0::SHIFTBUFNIS;
}

/// Shifter Buffer N Odd Even Swapped Register
pub mod SHIFTBUFOES0 {

    /// Shift Buffer
    pub mod SHIFTBUFOES {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Odd Even Swapped Register
pub mod SHIFTBUFOES1 {
    pub use super::SHIFTBUFOES0::SHIFTBUFOES;
}

/// Shifter Buffer N Odd Even Swapped Register
pub mod SHIFTBUFOES2 {
    pub use super::SHIFTBUFOES0::SHIFTBUFOES;
}

/// Shifter Buffer N Odd Even Swapped Register
pub mod SHIFTBUFOES3 {
    pub use super::SHIFTBUFOES0::SHIFTBUFOES;
}

/// Shifter Buffer N Odd Even Swapped Register
pub mod SHIFTBUFOES4 {
    pub use super::SHIFTBUFOES0::SHIFTBUFOES;
}

/// Shifter Buffer N Odd Even Swapped Register
pub mod SHIFTBUFOES5 {
    pub use super::SHIFTBUFOES0::SHIFTBUFOES;
}

/// Shifter Buffer N Odd Even Swapped Register
pub mod SHIFTBUFOES6 {
    pub use super::SHIFTBUFOES0::SHIFTBUFOES;
}

/// Shifter Buffer N Odd Even Swapped Register
pub mod SHIFTBUFOES7 {
    pub use super::SHIFTBUFOES0::SHIFTBUFOES;
}

/// Shifter Buffer N Even Odd Swapped Register
pub mod SHIFTBUFEOS0 {

    /// Shift Buffer
    pub mod SHIFTBUFEOS {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
        pub const mask: u32 = 0xffffffff << offset;
        /// Read-only values (empty)
        pub mod R {}
        /// Write-only values (empty)
        pub mod W {}
        /// Read-write values (empty)
        pub mod RW {}
    }
}

/// Shifter Buffer N Even Odd Swapped Register
pub mod SHIFTBUFEOS1 {
    pub use super::SHIFTBUFEOS0::SHIFTBUFEOS;
}

/// Shifter Buffer N Even Odd Swapped Register
pub mod SHIFTBUFEOS2 {
    pub use super::SHIFTBUFEOS0::SHIFTBUFEOS;
}

/// Shifter Buffer N Even Odd Swapped Register
pub mod SHIFTBUFEOS3 {
    pub use super::SHIFTBUFEOS0::SHIFTBUFEOS;
}

/// Shifter Buffer N Even Odd Swapped Register
pub mod SHIFTBUFEOS4 {
    pub use super::SHIFTBUFEOS0::SHIFTBUFEOS;
}

/// Shifter Buffer N Even Odd Swapped Register
pub mod SHIFTBUFEOS5 {
    pub use super::SHIFTBUFEOS0::SHIFTBUFEOS;
}

/// Shifter Buffer N Even Odd Swapped Register
pub mod SHIFTBUFEOS6 {
    pub use super::SHIFTBUFEOS0::SHIFTBUFEOS;
}

/// Shifter Buffer N Even Odd Swapped Register
pub mod SHIFTBUFEOS7 {
    pub use super::SHIFTBUFEOS0::SHIFTBUFEOS;
}
#[repr(C)]
pub struct RegisterBlock {
    /// Version ID Register
    pub VERID: RORegister<u32>,

    /// Parameter Register
    pub PARAM: RORegister<u32>,

    /// FlexIO Control Register
    pub CTRL: RWRegister<u32>,

    /// Pin State Register
    pub PIN: RORegister<u32>,

    /// Shifter Status Register
    pub SHIFTSTAT: RWRegister<u32>,

    /// Shifter Error Register
    pub SHIFTERR: RWRegister<u32>,

    /// Timer Status Register
    pub TIMSTAT: RWRegister<u32>,

    _reserved1: [u32; 1],

    /// Shifter Status Interrupt Enable
    pub SHIFTSIEN: RWRegister<u32>,

    /// Shifter Error Interrupt Enable
    pub SHIFTEIEN: RWRegister<u32>,

    /// Timer Interrupt Enable Register
    pub TIMIEN: RWRegister<u32>,

    _reserved2: [u32; 1],

    /// Shifter Status DMA Enable
    pub SHIFTSDEN: RWRegister<u32>,

    _reserved3: [u32; 1],

    /// Timer Status DMA Enable
    pub TIMERSDEN: RWRegister<u32>,

    _reserved4: [u32; 1],

    /// Shifter State Register
    pub SHIFTSTATE: RWRegister<u32>,

    _reserved5: [u32; 15],

    /// Shifter Control N Register
    pub SHIFTCTL0: RWRegister<u32>,

    /// Shifter Control N Register
    pub SHIFTCTL1: RWRegister<u32>,

    /// Shifter Control N Register
    pub SHIFTCTL2: RWRegister<u32>,

    /// Shifter Control N Register
    pub SHIFTCTL3: RWRegister<u32>,

    /// Shifter Control N Register
    pub SHIFTCTL4: RWRegister<u32>,

    /// Shifter Control N Register
    pub SHIFTCTL5: RWRegister<u32>,

    /// Shifter Control N Register
    pub SHIFTCTL6: RWRegister<u32>,

    /// Shifter Control N Register
    pub SHIFTCTL7: RWRegister<u32>,

    _reserved6: [u32; 24],

    /// Shifter Configuration N Register
    pub SHIFTCFG0: RWRegister<u32>,

    /// Shifter Configuration N Register
    pub SHIFTCFG1: RWRegister<u32>,

    /// Shifter Configuration N Register
    pub SHIFTCFG2: RWRegister<u32>,

    /// Shifter Configuration N Register
    pub SHIFTCFG3: RWRegister<u32>,

    /// Shifter Configuration N Register
    pub SHIFTCFG4: RWRegister<u32>,

    /// Shifter Configuration N Register
    pub SHIFTCFG5: RWRegister<u32>,

    /// Shifter Configuration N Register
    pub SHIFTCFG6: RWRegister<u32>,

    /// Shifter Configuration N Register
    pub SHIFTCFG7: RWRegister<u32>,

    _reserved7: [u32; 56],

    /// Shifter Buffer N Register
    pub SHIFTBUF0: RWRegister<u32>,

    /// Shifter Buffer N Register
    pub SHIFTBUF1: RWRegister<u32>,

    /// Shifter Buffer N Register
    pub SHIFTBUF2: RWRegister<u32>,

    /// Shifter Buffer N Register
    pub SHIFTBUF3: RWRegister<u32>,

    /// Shifter Buffer N Register
    pub SHIFTBUF4: RWRegister<u32>,

    /// Shifter Buffer N Register
    pub SHIFTBUF5: RWRegister<u32>,

    /// Shifter Buffer N Register
    pub SHIFTBUF6: RWRegister<u32>,

    /// Shifter Buffer N Register
    pub SHIFTBUF7: RWRegister<u32>,

    _reserved8: [u32; 24],

    /// Shifter Buffer N Bit Swapped Register
    pub SHIFTBUFBIS0: RWRegister<u32>,

    /// Shifter Buffer N Bit Swapped Register
    pub SHIFTBUFBIS1: RWRegister<u32>,

    /// Shifter Buffer N Bit Swapped Register
    pub SHIFTBUFBIS2: RWRegister<u32>,

    /// Shifter Buffer N Bit Swapped Register
    pub SHIFTBUFBIS3: RWRegister<u32>,

    /// Shifter Buffer N Bit Swapped Register
    pub SHIFTBUFBIS4: RWRegister<u32>,

    /// Shifter Buffer N Bit Swapped Register
    pub SHIFTBUFBIS5: RWRegister<u32>,

    /// Shifter Buffer N Bit Swapped Register
    pub SHIFTBUFBIS6: RWRegister<u32>,

    /// Shifter Buffer N Bit Swapped Register
    pub SHIFTBUFBIS7: RWRegister<u32>,

    _reserved9: [u32; 24],

    /// Shifter Buffer N Byte Swapped Register
    pub SHIFTBUFBYS0: RWRegister<u32>,

    /// Shifter Buffer N Byte Swapped Register
    pub SHIFTBUFBYS1: RWRegister<u32>,

    /// Shifter Buffer N Byte Swapped Register
    pub SHIFTBUFBYS2: RWRegister<u32>,

    /// Shifter Buffer N Byte Swapped Register
    pub SHIFTBUFBYS3: RWRegister<u32>,

    /// Shifter Buffer N Byte Swapped Register
    pub SHIFTBUFBYS4: RWRegister<u32>,

    /// Shifter Buffer N Byte Swapped Register
    pub SHIFTBUFBYS5: RWRegister<u32>,

    /// Shifter Buffer N Byte Swapped Register
    pub SHIFTBUFBYS6: RWRegister<u32>,

    /// Shifter Buffer N Byte Swapped Register
    pub SHIFTBUFBYS7: RWRegister<u32>,

    _reserved10: [u32; 24],

    /// Shifter Buffer N Bit Byte Swapped Register
    pub SHIFTBUFBBS0: RWRegister<u32>,

    /// Shifter Buffer N Bit Byte Swapped Register
    pub SHIFTBUFBBS1: RWRegister<u32>,

    /// Shifter Buffer N Bit Byte Swapped Register
    pub SHIFTBUFBBS2: RWRegister<u32>,

    /// Shifter Buffer N Bit Byte Swapped Register
    pub SHIFTBUFBBS3: RWRegister<u32>,

    /// Shifter Buffer N Bit Byte Swapped Register
    pub SHIFTBUFBBS4: RWRegister<u32>,

    /// Shifter Buffer N Bit Byte Swapped Register
    pub SHIFTBUFBBS5: RWRegister<u32>,

    /// Shifter Buffer N Bit Byte Swapped Register
    pub SHIFTBUFBBS6: RWRegister<u32>,

    /// Shifter Buffer N Bit Byte Swapped Register
    pub SHIFTBUFBBS7: RWRegister<u32>,

    _reserved11: [u32; 24],

    /// Timer Control N Register
    pub TIMCTL0: RWRegister<u32>,

    /// Timer Control N Register
    pub TIMCTL1: RWRegister<u32>,

    /// Timer Control N Register
    pub TIMCTL2: RWRegister<u32>,

    /// Timer Control N Register
    pub TIMCTL3: RWRegister<u32>,

    /// Timer Control N Register
    pub TIMCTL4: RWRegister<u32>,

    /// Timer Control N Register
    pub TIMCTL5: RWRegister<u32>,

    /// Timer Control N Register
    pub TIMCTL6: RWRegister<u32>,

    /// Timer Control N Register
    pub TIMCTL7: RWRegister<u32>,

    _reserved12: [u32; 24],

    /// Timer Configuration N Register
    pub TIMCFG0: RWRegister<u32>,

    /// Timer Configuration N Register
    pub TIMCFG1: RWRegister<u32>,

    /// Timer Configuration N Register
    pub TIMCFG2: RWRegister<u32>,

    /// Timer Configuration N Register
    pub TIMCFG3: RWRegister<u32>,

    /// Timer Configuration N Register
    pub TIMCFG4: RWRegister<u32>,

    /// Timer Configuration N Register
    pub TIMCFG5: RWRegister<u32>,

    /// Timer Configuration N Register
    pub TIMCFG6: RWRegister<u32>,

    /// Timer Configuration N Register
    pub TIMCFG7: RWRegister<u32>,

    _reserved13: [u32; 24],

    /// Timer Compare N Register
    pub TIMCMP0: RWRegister<u32>,

    /// Timer Compare N Register
    pub TIMCMP1: RWRegister<u32>,

    /// Timer Compare N Register
    pub TIMCMP2: RWRegister<u32>,

    /// Timer Compare N Register
    pub TIMCMP3: RWRegister<u32>,

    /// Timer Compare N Register
    pub TIMCMP4: RWRegister<u32>,

    /// Timer Compare N Register
    pub TIMCMP5: RWRegister<u32>,

    /// Timer Compare N Register
    pub TIMCMP6: RWRegister<u32>,

    /// Timer Compare N Register
    pub TIMCMP7: RWRegister<u32>,

    _reserved14: [u32; 88],

    /// Shifter Buffer N Nibble Byte Swapped Register
    pub SHIFTBUFNBS0: RWRegister<u32>,

    /// Shifter Buffer N Nibble Byte Swapped Register
    pub SHIFTBUFNBS1: RWRegister<u32>,

    /// Shifter Buffer N Nibble Byte Swapped Register
    pub SHIFTBUFNBS2: RWRegister<u32>,

    /// Shifter Buffer N Nibble Byte Swapped Register
    pub SHIFTBUFNBS3: RWRegister<u32>,

    /// Shifter Buffer N Nibble Byte Swapped Register
    pub SHIFTBUFNBS4: RWRegister<u32>,

    /// Shifter Buffer N Nibble Byte Swapped Register
    pub SHIFTBUFNBS5: RWRegister<u32>,

    /// Shifter Buffer N Nibble Byte Swapped Register
    pub SHIFTBUFNBS6: RWRegister<u32>,

    /// Shifter Buffer N Nibble Byte Swapped Register
    pub SHIFTBUFNBS7: RWRegister<u32>,

    _reserved15: [u32; 24],

    /// Shifter Buffer N Half Word Swapped Register
    pub SHIFTBUFHWS0: RWRegister<u32>,

    /// Shifter Buffer N Half Word Swapped Register
    pub SHIFTBUFHWS1: RWRegister<u32>,

    /// Shifter Buffer N Half Word Swapped Register
    pub SHIFTBUFHWS2: RWRegister<u32>,

    /// Shifter Buffer N Half Word Swapped Register
    pub SHIFTBUFHWS3: RWRegister<u32>,

    /// Shifter Buffer N Half Word Swapped Register
    pub SHIFTBUFHWS4: RWRegister<u32>,

    /// Shifter Buffer N Half Word Swapped Register
    pub SHIFTBUFHWS5: RWRegister<u32>,

    /// Shifter Buffer N Half Word Swapped Register
    pub SHIFTBUFHWS6: RWRegister<u32>,

    /// Shifter Buffer N Half Word Swapped Register
    pub SHIFTBUFHWS7: RWRegister<u32>,

    _reserved16: [u32; 24],

    /// Shifter Buffer N Nibble Swapped Register
    pub SHIFTBUFNIS0: RWRegister<u32>,

    /// Shifter Buffer N Nibble Swapped Register
    pub SHIFTBUFNIS1: RWRegister<u32>,

    /// Shifter Buffer N Nibble Swapped Register
    pub SHIFTBUFNIS2: RWRegister<u32>,

    /// Shifter Buffer N Nibble Swapped Register
    pub SHIFTBUFNIS3: RWRegister<u32>,

    /// Shifter Buffer N Nibble Swapped Register
    pub SHIFTBUFNIS4: RWRegister<u32>,

    /// Shifter Buffer N Nibble Swapped Register
    pub SHIFTBUFNIS5: RWRegister<u32>,

    /// Shifter Buffer N Nibble Swapped Register
    pub SHIFTBUFNIS6: RWRegister<u32>,

    /// Shifter Buffer N Nibble Swapped Register
    pub SHIFTBUFNIS7: RWRegister<u32>,

    _reserved17: [u32; 24],

    /// Shifter Buffer N Odd Even Swapped Register
    pub SHIFTBUFOES0: RWRegister<u32>,

    /// Shifter Buffer N Odd Even Swapped Register
    pub SHIFTBUFOES1: RWRegister<u32>,

    /// Shifter Buffer N Odd Even Swapped Register
    pub SHIFTBUFOES2: RWRegister<u32>,

    /// Shifter Buffer N Odd Even Swapped Register
    pub SHIFTBUFOES3: RWRegister<u32>,

    /// Shifter Buffer N Odd Even Swapped Register
    pub SHIFTBUFOES4: RWRegister<u32>,

    /// Shifter Buffer N Odd Even Swapped Register
    pub SHIFTBUFOES5: RWRegister<u32>,

    /// Shifter Buffer N Odd Even Swapped Register
    pub SHIFTBUFOES6: RWRegister<u32>,

    /// Shifter Buffer N Odd Even Swapped Register
    pub SHIFTBUFOES7: RWRegister<u32>,

    _reserved18: [u32; 24],

    /// Shifter Buffer N Even Odd Swapped Register
    pub SHIFTBUFEOS0: RWRegister<u32>,

    /// Shifter Buffer N Even Odd Swapped Register
    pub SHIFTBUFEOS1: RWRegister<u32>,

    /// Shifter Buffer N Even Odd Swapped Register
    pub SHIFTBUFEOS2: RWRegister<u32>,

    /// Shifter Buffer N Even Odd Swapped Register
    pub SHIFTBUFEOS3: RWRegister<u32>,

    /// Shifter Buffer N Even Odd Swapped Register
    pub SHIFTBUFEOS4: RWRegister<u32>,

    /// Shifter Buffer N Even Odd Swapped Register
    pub SHIFTBUFEOS5: RWRegister<u32>,

    /// Shifter Buffer N Even Odd Swapped Register
    pub SHIFTBUFEOS6: RWRegister<u32>,

    /// Shifter Buffer N Even Odd Swapped Register
    pub SHIFTBUFEOS7: RWRegister<u32>,
}
pub struct ResetValues {
    pub VERID: u32,
    pub PARAM: u32,
    pub CTRL: u32,
    pub PIN: u32,
    pub SHIFTSTAT: u32,
    pub SHIFTERR: u32,
    pub TIMSTAT: u32,
    pub SHIFTSIEN: u32,
    pub SHIFTEIEN: u32,
    pub TIMIEN: u32,
    pub SHIFTSDEN: u32,
    pub TIMERSDEN: u32,
    pub SHIFTSTATE: u32,
    pub SHIFTCTL0: u32,
    pub SHIFTCTL1: u32,
    pub SHIFTCTL2: u32,
    pub SHIFTCTL3: u32,
    pub SHIFTCTL4: u32,
    pub SHIFTCTL5: u32,
    pub SHIFTCTL6: u32,
    pub SHIFTCTL7: u32,
    pub SHIFTCFG0: u32,
    pub SHIFTCFG1: u32,
    pub SHIFTCFG2: u32,
    pub SHIFTCFG3: u32,
    pub SHIFTCFG4: u32,
    pub SHIFTCFG5: u32,
    pub SHIFTCFG6: u32,
    pub SHIFTCFG7: u32,
    pub SHIFTBUF0: u32,
    pub SHIFTBUF1: u32,
    pub SHIFTBUF2: u32,
    pub SHIFTBUF3: u32,
    pub SHIFTBUF4: u32,
    pub SHIFTBUF5: u32,
    pub SHIFTBUF6: u32,
    pub SHIFTBUF7: u32,
    pub SHIFTBUFBIS0: u32,
    pub SHIFTBUFBIS1: u32,
    pub SHIFTBUFBIS2: u32,
    pub SHIFTBUFBIS3: u32,
    pub SHIFTBUFBIS4: u32,
    pub SHIFTBUFBIS5: u32,
    pub SHIFTBUFBIS6: u32,
    pub SHIFTBUFBIS7: u32,
    pub SHIFTBUFBYS0: u32,
    pub SHIFTBUFBYS1: u32,
    pub SHIFTBUFBYS2: u32,
    pub SHIFTBUFBYS3: u32,
    pub SHIFTBUFBYS4: u32,
    pub SHIFTBUFBYS5: u32,
    pub SHIFTBUFBYS6: u32,
    pub SHIFTBUFBYS7: u32,
    pub SHIFTBUFBBS0: u32,
    pub SHIFTBUFBBS1: u32,
    pub SHIFTBUFBBS2: u32,
    pub SHIFTBUFBBS3: u32,
    pub SHIFTBUFBBS4: u32,
    pub SHIFTBUFBBS5: u32,
    pub SHIFTBUFBBS6: u32,
    pub SHIFTBUFBBS7: u32,
    pub TIMCTL0: u32,
    pub TIMCTL1: u32,
    pub TIMCTL2: u32,
    pub TIMCTL3: u32,
    pub TIMCTL4: u32,
    pub TIMCTL5: u32,
    pub TIMCTL6: u32,
    pub TIMCTL7: u32,
    pub TIMCFG0: u32,
    pub TIMCFG1: u32,
    pub TIMCFG2: u32,
    pub TIMCFG3: u32,
    pub TIMCFG4: u32,
    pub TIMCFG5: u32,
    pub TIMCFG6: u32,
    pub TIMCFG7: u32,
    pub TIMCMP0: u32,
    pub TIMCMP1: u32,
    pub TIMCMP2: u32,
    pub TIMCMP3: u32,
    pub TIMCMP4: u32,
    pub TIMCMP5: u32,
    pub TIMCMP6: u32,
    pub TIMCMP7: u32,
    pub SHIFTBUFNBS0: u32,
    pub SHIFTBUFNBS1: u32,
    pub SHIFTBUFNBS2: u32,
    pub SHIFTBUFNBS3: u32,
    pub SHIFTBUFNBS4: u32,
    pub SHIFTBUFNBS5: u32,
    pub SHIFTBUFNBS6: u32,
    pub SHIFTBUFNBS7: u32,
    pub SHIFTBUFHWS0: u32,
    pub SHIFTBUFHWS1: u32,
    pub SHIFTBUFHWS2: u32,
    pub SHIFTBUFHWS3: u32,
    pub SHIFTBUFHWS4: u32,
    pub SHIFTBUFHWS5: u32,
    pub SHIFTBUFHWS6: u32,
    pub SHIFTBUFHWS7: u32,
    pub SHIFTBUFNIS0: u32,
    pub SHIFTBUFNIS1: u32,
    pub SHIFTBUFNIS2: u32,
    pub SHIFTBUFNIS3: u32,
    pub SHIFTBUFNIS4: u32,
    pub SHIFTBUFNIS5: u32,
    pub SHIFTBUFNIS6: u32,
    pub SHIFTBUFNIS7: u32,
    pub SHIFTBUFOES0: u32,
    pub SHIFTBUFOES1: u32,
    pub SHIFTBUFOES2: u32,
    pub SHIFTBUFOES3: u32,
    pub SHIFTBUFOES4: u32,
    pub SHIFTBUFOES5: u32,
    pub SHIFTBUFOES6: u32,
    pub SHIFTBUFOES7: u32,
    pub SHIFTBUFEOS0: u32,
    pub SHIFTBUFEOS1: u32,
    pub SHIFTBUFEOS2: u32,
    pub SHIFTBUFEOS3: u32,
    pub SHIFTBUFEOS4: u32,
    pub SHIFTBUFEOS5: u32,
    pub SHIFTBUFEOS6: u32,
    pub SHIFTBUFEOS7: u32,
}
pub struct Instance<const N: u8> {
    #[cfg_attr(feature = "nosync", allow(unused))]
    pub(crate) addr: u32,
    #[cfg_attr(feature = "nosync", allow(unused))]
    pub(crate) intrs: &'static [crate::Interrupt],
}

#[cfg(not(feature = "nosync"))]
impl<const N: u8> ::core::ops::Deref for Instance<N> {
    type Target = RegisterBlock;
    #[inline(always)]
    fn deref(&self) -> &RegisterBlock {
        unsafe { &*(self.addr as *const _) }
    }
}

#[cfg(not(feature = "nosync"))]
unsafe impl<const N: u8> Send for Instance<N> {}

#[cfg(not(feature = "nosync"))]
impl<const N: u8> Instance<N> {
    /// Return the interrupt signals associated with this
    /// peripheral instance
    ///
    /// Collection may be empty if there is no interrupt signal
    /// associated with the peripheral. There's no guarantee for
    /// interrupt signal ordering in the collection.
    #[inline(always)]
    pub const fn interrupts<'a>(&'a self) -> &'a [crate::Interrupt] {
        self.intrs
    }
}
