...
use crate::{RORegister, RWRegister, UnsafeWORegister, WORegister};
...
/// The SystemControl peripheral instance.
#[cfg(not(feature = "doc"))]
pub type SystemControl = Instance<0>;
...
#[no_mangle]
static SystemControl_TAKEN: AtomicBool = AtomicBool::new(false);
...
impl SystemControl {
    ...
    pub fn take() -> Option<Self> {
        let taken = SystemControl_TAKEN.swap(true, Ordering::SeqCst);
        if taken {
            None
        } else {
            Some(Self::INSTANCE)
    ...
    pub fn release(_: Self) {
        let taken = SystemControl_TAKEN.swap(false, Ordering::SeqCst);
    ...
    pub unsafe fn steal() -> Self {
        SystemControl_TAKEN.store(true, Ordering::SeqCst);
        Self::INSTANCE
    }
}

impl SystemControl {
    /// The interrupts associated with SystemControl
    #[cfg(not(feature = "doc"))]
